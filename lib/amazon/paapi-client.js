...
class PaapiClient {
  constructor() {
    // Configuración PA-API con validaciones
  ...
  validateConfig() {
    const missing = [];
    if (!this.accessKey) missing.push('AWS_ACCESS_KEY_ID');
    if (!this.secretKey) missing.push('AWS_SECRET_ACCESS_KEY');
    if (!this.partnerTag) missing.push('AMAZON_PARTNER_TAG');
    ...
    console.log('✅ Configuración PA-API validada');
  }
