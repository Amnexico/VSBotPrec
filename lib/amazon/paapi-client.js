...
class PaapiClient {
  ...
  // Crear payload con recursos básicos válidos
  createPayload(asin) {
    return {
      ItemIds: [asin],
      Resources: [
  ...
  // Parser básico para extraer información con recursos limitados
  parseResponse(response, asin) {
    try {
      ...
      // Usar la primera oferta disponible (sin filtrado avanzado por ahora)
      const firstOffer = item.Offers?.Listings?.[0] || null;
      
      // Extraer información básica del vendedor (limitada)
      let sellerInfo = this.getBasicSellerInfo(firstOffer);
      ...
      if (firstOffer?.Availability?.Message) {
        availability = firstOffer.Availability.Message;
      }
  ...
  // MÉTODO BÁSICO: Obtener información básica del vendedor
  getBasicSellerInfo(offer) {
    if (!offer) {
      return {
        type: 'unknown',
    ...
    // Sin acceso a MerchantInfo, usamos detección básica
    // Por ahora, asumimos Amazon directo si no hay indicaciones contrarias
    return {
      type: 'amazon_general',
      name: 'Amazon',
      emoji: '✅'
