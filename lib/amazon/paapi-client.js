...
class PaapiClient {
  constructor() {
    ...
    // Configuración fija para Amazon España - CORREGIDA
    this.host = 'webservices.amazon.es';
    this.region = 'eu-west-1';
    this.service = 'ProductAdvertisingAPIv1'; // Cambio crítico para España
    
    console.log('=== CONFIGURACIÓN INICIAL ===');
    console.log('Host configurado:', this.host);
    console.log('Region configurada:', this.region);
    console.log('Service configurado:', this.service);
  ...
  createSignature(method, uri, queryString, headers, payload, timestamp) {
    ...
    // Task 1: Create canonical request - ORDEN CRÍTICO
    const headerKeys = Object.keys(headers).map(key => key.toLowerCase()).sort();
    ...
    const canonicalRequest = [
      method,
      uri,
      queryString || '', // Asegurar que hay una línea vacía si no hay query string
      canonicalHeaders,
      signedHeaders,
      payloadHash
    ].join('\n');
    ...
    console.log('Full String to Sign:');
  ...
  async makeRequest(operation, payload) {
    ...
    const queryString = ''; // Explícitamente vacío para POST
