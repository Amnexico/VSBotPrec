...
class PaapiClient {
        ...
        const req = https.get(options, (res) => {
          ...
          res.on('end', () => {
            try {
              ...
              // Extraer datos básicos con cheerio
              const productData = {
                asin: asin,
                name: $('#productTitle').text().trim() || 'Producto sin nombre',
  ...
  // Extraer precio de HTML (implementación básica)
  ...
  // Extraer disponibilidad de HTML (implementación básica)
  extractAvailabilityFromPage($) {
    ...
    for (const selector of availabilitySelectors) {
      ...
      if (availText && availText.length > 0) {
        return availText;
      }
    }
    return 'Disponible';
