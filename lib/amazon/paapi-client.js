...
class PaapiClient {
  ...
  // Payload optimizado con TODOS los recursos de descuentos disponibles
  createPayload(asin) {
    return {
      ItemIds: [asin],
      Resources: [
        ...
        // RECURSOS OFFERS OPTIMIZADOS PARA DESCUENTOS
        ...
        // RECURSOS OFFERSV2 PARA DEALS Y PROMOCIONES AVANZADAS
        'OffersV2.Listings.Deal.Badge',
        'OffersV2.Listings.Deal.DiscountPercent',
        'OffersV2.Listings.Deal.EndTime', 
        'OffersV2.Listings.Deal.StartTime',
        'OffersV2.Listings.Deal.Type',
  ...
  parseResponse(response, asin) {
    try {
      ...
      // También revisar OffersV2 si está disponible
      const offersV2 = item.OffersV2?.Listings || [];
      const bestOfferV2 = offersV2.length > 0 ? offersV2[0] : null;
      
      console.log('Mejor oferta (Offers):', JSON.stringify(bestOffer, null, 2));
      console.log('Mejor oferta (OffersV2):', JSON.stringify(bestOfferV2, null, 2));
      ...
      } else if (bestOfferV2?.Price) {
        finalPrice = bestOfferV2.Price.Amount || 0;
        displayPrice = finalPrice;
        currency = bestOfferV2.Price.Currency || 'EUR';
      }
      
      // PASO 2: Extraer precio original (SavingBasis)
      if (bestOffer?.SavingBasis) {
        listPrice = bestOffer.SavingBasis.Amount || 0;
      } else if (bestOfferV2?.SavingBasis) {
        listPrice = bestOfferV2.SavingBasis.Amount || 0;
      }
      ...
      // PASO 4: Analizar deals en OffersV2
      if (bestOfferV2?.Deal) {
        console.log('=== DEAL OFFERSV2 DETECTADO ===');
        console.log(JSON.stringify(bestOfferV2.Deal, null, 2));
        
        dealInfo = {
          type: bestOfferV2.Deal.Type || 'Deal',
          badge: bestOfferV2.Deal.Badge || 'Oferta especial',
          discountPercent: bestOfferV2.Deal.DiscountPercent || 0,
          startTime: bestOfferV2.Deal.StartTime,
          endTime: bestOfferV2.Deal.EndTime
        };
        
        if (dealInfo.discountPercent > 0) {
          const dealDiscount = (finalPrice * dealInfo.discountPercent) / 100;
          promotionDetails.push({
            type: 'Deal',
            description: `${dealInfo.badge} (${dealInfo.discountPercent}%)`,
            amount: dealDiscount,
            source: 'OffersV2'
          });
        }
      ...
      // PASO 5: Analizar promociones en OffersV2
      if (bestOfferV2?.Promotions) {
        console.log('=== PROMOCIONES OFFERSV2 DETECTADAS ===');
        console.log(JSON.stringify(bestOfferV2.Promotions, null, 2));
        
        bestOfferV2.Promotions.forEach((promo, index) => {
          console.log(`Promoción V2 ${index + 1}:`, JSON.stringify(promo, null, 2));
          ...
          promotionDetails.push({
            type: promo.Type || 'Promoción V2',
            description: promo.DiscountDisplayAmount || 'Promoción especial',
            amount: discountAmount,
            source: 'OffersV2'
          });
        });
