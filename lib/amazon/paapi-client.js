...
class PaapiClient {
  ...
  // Método para resolver enlaces acortados de Amazon
  async resolveShortUrl(shortUrl) {
    return new Promise((resolve, reject) => {
      const options = {
        hostname: shortUrl.includes('amzn.eu') ? 'amzn.eu' : 'amzn.to',
        path: shortUrl.split('.eu')[1] || shortUrl.split('.to')[1] || '/',
        method: 'HEAD', // Solo queremos los headers, no el contenido
        timeout: 5000
      };

      const req = https.request(options, (res) => {
        // Amazon redirige con código 301 o 302
        if (res.statusCode === 301 || res.statusCode === 302) {
          const location = res.headers.location;
          if (location) {
            console.log(`Enlace resuelto: ${shortUrl} → ${location}`);
            resolve(location);
          } else {
            reject(new Error('No se encontró la redirección'));
          }
        } else {
          reject(new Error(`Código de respuesta inesperado: ${res.statusCode}`));
        }
      });

      req.on('error', (error) => {
        console.error(`Error resolviendo enlace acortado ${shortUrl}:`, error.message);
        reject(error);
      });

      req.setTimeout(5000, () => {
        req.destroy();
        reject(new Error('Timeout resolviendo enlace acortado'));
      });
