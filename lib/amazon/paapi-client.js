...
class PaapiClient {
  constructor() {
    ...
    this.host = 'webservices.amazon.com'; // Cambio crítico: probar US marketplace
    this.region = 'us-east-1';
    this.service = 'ProductAdvertisingAPI'; // Volver al service original
    
    console.log('PA-API Client inicializado con aws4 - US marketplace test');
  }
  ...
  async makeRequest(operation, payload) {
    ...
    const options = {
      host: this.host,
      path: '/paapi5/getitems', // Volver al path original con nuevo service
      method: 'POST',
      headers: {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            console.error('Raw response (HTML):', data.substring(0, 500)); // Mostrar HTML response
  ...
  async getProductInfo(asin) {
    const payload = {
      ...
      Marketplace: 'www.amazon.com', // Cambio a US marketplace
      ItemIds: [asin],
      Resources: [
        'ItemInfo.Title',
  ...
  async getProductByUrl(url) {
    ...
    try {
      ...
      return {
        ...
        image: '',
        affiliateUrl: `https://www.amazon.com/dp/${asin}?tag=${this.trackingTag}` // URL para amazon.com
      };
    }
  }
}
