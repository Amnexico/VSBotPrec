...
const ayudaAdmin = async (ctx) => {
  ...
  try {
    // MENSAJE 1: ROBOTS ASPIRADORES Y COMUNICACIÓN
...
- \`/emailblast Ofertas|Hola, ofertas de la semana\``;
    ...
    await ctx.replyWithMarkdown(mensaje1);
    
    // MENSAJE 2: GESTIÓN DE PRECIOS
...
/recuperarproducto ASIN - Recuperar producto eliminado`;
    ...
    await ctx.replyWithMarkdown(mensaje2);
    
    // MENSAJE 3: IMPORTACIÓN Y DIAGNÓSTICO
...
- \`/testearalerta B0DCVYS9FQ\``;
    ...
    await ctx.replyWithMarkdown(mensaje3);
    
    // MENSAJE 4: FLUJO DE TRABAJO Y FORMATO
...
✅ **Ayuda completa enviada en 4 partes**`;
    ...
    await ctx.replyWithMarkdown(mensaje4);
...
const forzaroferta = async (ctx) => {
  ...
  try {
    ...
    // CAMBIO: Crear instancia del servicio con el bot
    const OfferService = require('../../services/offer-service');
    const offerService = new OfferService(ctx.telegram);
    
    // CAMBIO: Usar el método correcto
