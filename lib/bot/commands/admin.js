...
const ayudaAdmin = async (ctx) => {
  ...
  try {
    // MENSAJE 1: ROBOTS Y COMUNICACIÓN (SIN MARKDOWN)
    const mensaje1 = `📋 COMANDOS DE ADMINISTRADOR - PARTE 1/5

🤖 ROBOTS ASPIRADORES:
/marcarrobot ASIN [nombre_personalizado] - Marcar producto como robot
...
/testconfiguracion - Diagnosticar sistema de ofertas

Ejemplos:
- /marcarrobot B0DCVYS9FQ
- /marcarrobot B0DCVYS9FQ "DREAME L40s Pro Ultra"
...
- /emailblast Ofertas|Hola, ofertas de la semana`;

    await ctx.reply(mensaje1);
    
    // MENSAJE 2: GESTIÓN DE PRECIOS (SIN MARKDOWN)
    const mensaje2 = `📋 COMANDOS DE ADMINISTRADOR - PARTE 2/5
...
Ejemplos:
- /agregarprecio B0DCVYS9FQ 339.99 "Oferta Flash"
...
/corregirnombres - Corregir nombres faltantes
/recuperarproducto ASIN - Recuperar producto eliminado`;

    await ctx.reply(mensaje2);
    
    // MENSAJE 3: GRUPOS DE PRODUCTOS (NUEVO)
    const mensaje3 = `📋 COMANDOS DE ADMINISTRADOR - PARTE 3/5

📦 GRUPOS DE PRODUCTOS:
/agrupar ASIN1 ASIN2 [ASIN3...] "Nombre" - Agrupar variantes
/desagrupar GRUPO_ID - Eliminar grupo
/vergrupos - Listar todos los grupos

Ejemplos:
- /agrupar B0DCVYS9FQ B0DCVYS9FR "DREAME L40s Pro Ultra"
- /desagrupar dreame-l40s-pro-ultra
- /vergrupos

💡 Los grupos permiten:
• Alertas cruzadas entre variantes (blanco/negro/gris)
• Usuario con ASIN blanco recibe alerta si baja el negro
• Unificación de productos con mismo modelo, distintos colores
• Gestión centralizada de variantes`;

    await ctx.reply(mensaje3);
    
    // MENSAJE 4: IMPORTACIÓN Y DIAGNÓSTICO (SIN MARKDOWN)
    const mensaje4 = `📋 COMANDOS DE ADMINISTRADOR - PARTE 4/5
...
- /testearalerta B0DCVYS9FQ`;
    ...
    await ctx.reply(mensaje4);
    
    // MENSAJE 5: FLUJO DE TRABAJO (SIN MARKDOWN)
    const mensaje5 = `📋 COMANDOS DE ADMINISTRADOR - PARTE 5/5

📅 FORMATO FECHAS: DD/MM/YYYY (ej: 28/08/2025)

🚀 FLUJO PARA OFERTAS AUTOMÁTICAS:
1. /marcarrobot B0DCVYS9FQ "Nombre corto" - Marcar como robot
2. Sistema detecta bajadas automáticamente cada 10 min
3. Se publican ofertas en @vacuumspain y @vacuumspain_ofertas
4. /forzaroferta B0DCVYS9FQ - Forzar publicación manual

🔗 FLUJO PARA GRUPOS:
1. /agrupar ASIN1 ASIN2 "Producto" - Crear grupo de variantes
2. Alertas automáticas cruzadas entre variantes
3. Usuario con blanco recibe alerta si baja negro
4. /vergrupos - Verificar grupos activos

💡 COMANDOS MAS USADOS:
- /testconfiguracion - Verificar sistema funcionando
- /resumenbot - Vista rápida diaria
- /diagnosticoasin ASIN - Resolver problemas  
- /agregarprecio ASIN precio comentario - Actualizar precios
- /vergrupos - Gestionar grupos de variantes

✅ Ayuda completa enviada en 5 partes`;

    await ctx.reply(mensaje5);
    
  } catch (error) {
    console.error('Error en ayudaAdmin:', error);
    ctx.reply(`❌ Error enviando ayuda: ${error.message}`);
  }
};
...
/listarobots [página] - Robots aspiradores marcados

**Ejemplos de uso:**
• \`/stats\` - Dashboard completo del sistema
• \`/resumenbot\` - Vista rápida recomendada para uso diario
• \`/listausuarios 1\` - Primera página de usuarios
• \`/listaproductos 2\` - Segunda página de productos
• \`/listarobots 1\` - Primera página de robots aspiradores

**Información mostrada:**
• **Dashboard:** Estadísticas generales, totales, alertas enviadas
• **Resumen:** Top usuarios, productos populares, métricas semanales
• **Listas:** Información detallada paginada para navegación fácil

**Tips:**
• Usa /resumenbot para revisión diaria rápida
• Las listas están ordenadas por relevancia (más usuarios/alertas primero)
• Los productos virales se marcan con ⚡
• Los robots aspiradores se muestran con 🤖`;
...
// ============================================================================
// AGREGAR ESTAS FUNCIONES A lib/bot/commands/admin.js
// ============================================================================

const agrupar = async (ctx) => {
  if (ctx.from.id !== ADMIN_USER_ID) {
    return ctx.reply('Comando no autorizado');
  }
  ...
  if (args.length < 4) {
    return ctx.reply('Uso: /agrupar ASIN1 ASIN2 [ASIN3...] "Nombre del producto"\n\nEjemplo: /agrupar B0DCVYS9FQ B0DCVYS9FR B0DCVYS9FS "DREAME L40s Pro Ultra"');
  }
  
  try {
    // Extraer nombre del producto (último argumento entre comillas)
    const fullText = ctx.message.text;
    const nameMatch = fullText.match(/"([^"]+)"$/);
    if (!nameMatch) {
      return ctx.reply('❌ El nombre del producto debe estar entre comillas al final\nEjemplo: "DREAME L40s Pro Ultra"');
    }
    
    const productName = nameMatch[1];
    
    // Extraer ASINs (todos los argumentos excepto el nombre)
    const textBeforeName = fullText.substring(0, fullText.lastIndexOf('"'));
    const asins = textBeforeName.split(' ').slice(1).filter(arg => arg.length === 10 && arg.match(/^[A-Z0-9]{10}$/));
    
    if (asins.length < 2) {
      return ctx.reply('❌ Se requieren al menos 2 ASINs válidos');
    }
    
    // Verificar que todos los ASINs existan en la base de datos
    const { Product, ProductGroup } = require('../../models');
    
    const existingProducts = await Product.find({ asin: { $in: asins } });
    const foundAsins = existingProducts.map(p => p.asin);
    const missingAsins = asins.filter(asin => !foundAsins.includes(asin));
    
    if (missingAsins.length > 0) {
      return ctx.reply(`❌ ASINs no encontrados en la base de datos: ${missingAsins.join(', ')}\n\nAgrega estos productos primero antes de agruparlos.`);
    }
    
    // Verificar que ningún ASIN ya pertenezca a un grupo
    const existingGroups = await ProductGroup.find({ variants: { $in: asins } });
    if (existingGroups.length > 0) {
      const conflictingAsins = existingGroups.flatMap(g => g.variants.filter(v => asins.includes(v)));
      return ctx.reply(`❌ Algunos ASINs ya pertenecen a grupos existentes: ${conflictingAsins.join(', ')}\n\nUsa /desagrupar primero si quieres reagruparlos.`);
    }
    
    // Crear ID del grupo
    const groupId = productName.toLowerCase()
      .replace(/[^a-z0-9\s]/g, '')
      .replace(/\s+/g, '-')
      .substring(0, 50);
    
    // Verificar si es robot aspirador (si algún ASIN del grupo lo es)
    const isRobotVacuum = existingProducts.some(p => p.isRobotVacuum);
    
    // Crear el grupo
    const productGroup = new ProductGroup({
      groupId: groupId,
      mainASIN: asins[0], // Primer ASIN como principal
      variants: asins,
      productName: productName,
      colors: new Map(), // Se puede completar manualmente después
      isRobotVacuum: isRobotVacuum,
      createdBy: ctx.from.id
    });
    
    await productGroup.save();
    
    let message = `✅ **Grupo de productos creado**\n\n`;
    message += `🏷️ ID: \`${groupId}\`\n`;
    message += `📦 Nombre: **${productName}**\n`;
    message += `🎯 ASIN principal: \`${asins[0]}\`\n`;
    message += `📊 Variantes: **${asins.length}**\n`;
    message += `${asins.map(asin => `   • \`${asin}\``).join('\n')}\n`;
    message += `🤖 Robot aspirador: ${isRobotVacuum ? '✅ Sí' : '❌ No'}\n\n`;
    message += `💡 **Próximos pasos:**\n`;
    message += `• Usa \`/colores ${groupId}\` para asignar colores\n`;
    message += `• Las alertas ahora serán cruzadas entre variantes\n`;
    message += `• Usa \`/vergrupos\` para listar todos los grupos`;
...
const desagrupar = async (ctx) => {
  ...
  const args = ctx.message.text.split(' ');
  if (args.length < 2) {
    return ctx.reply('Uso: /desagrupar GRUPO_ID\n\nEjemplo: /desagrupar dreame-l40s-pro-ultra\n\nUsa /vergrupos para ver IDs disponibles');
  }
  ...
  try {
    ...
    const group = await ProductGroup.findOne({ groupId: groupId });
    if (!group) {
      return ctx.reply(`❌ No se encontró el grupo: ${groupId}\n\nUsa /vergrupos para ver grupos disponibles`);
    }
    
    const deletedGroup = await ProductGroup.findByIdAndDelete(group._id);
    
    let message = `✅ **Grupo eliminado**\n\n`;
    message += `🏷️ ID: \`${groupId}\`\n`;
    message += `📦 Nombre: **${deletedGroup.productName}**\n`;
    message += `📊 Variantes eliminadas: **${deletedGroup.variants.length}**\n`;
    message += `${deletedGroup.variants.map(asin => `   • \`${asin}\``).join('\n')}\n\n`;
    message += `⚠️ Los productos individuales siguen activos\n`;
    message += `💡 Las alertas vuelven a ser independientes por ASIN`;
...
const vergrupos = async (ctx) => {
  ...
  try {
    ...
    const groups = await ProductGroup.find({}).sort({ createdAt: -1 });
    
    if (groups.length === 0) {
      return ctx.reply('📭 No hay grupos de productos creados\n\nUsa /agrupar para crear el primero');
    }
    
    let message = `📊 **GRUPOS DE PRODUCTOS** (${groups.length} total)\n\n`;
    
    for (const group of groups) {
      message += `🏷️ **${group.productName}**\n`;
      message += `📋 ID: \`${group.groupId}\`\n`;
      message += `🎯 Principal: \`${group.mainASIN}\`\n`;
      message += `📊 Variantes: ${group.variants.length} (${group.variants.join(', ')})\n`;
      message += `🤖 Robot: ${group.isRobotVacuum ? '✅' : '❌'}\n`;
      message += `📅 Creado: ${group.createdAt.toLocaleDateString('es-ES')}\n\n`;
    }
    
    message += `💡 **Comandos:**\n`;
    message += `• \`/desagrupar GRUPO_ID\` - Eliminar grupo\n`;
    message += `• \`/colores GRUPO_ID\` - Asignar colores (próximamente)`;
  ...
  try {
    ...
    if (products.length > 0) {
      ...
      for (const product of products) {
        ...
        // AGREGAR ESTA SECCIÓN NUEVA PARA OFERTAS AUTOMÁTICAS:
        // Si es un robot aspirador y hay bajada de precio significativa, publicar oferta
        if (product.isRobotVacuum && price < oldPrice) {
          const Bot = require('../../bot');
          const botInstance = Bot.getBotInstance();
          
          if (botInstance) {
            const OfferService = require('../../services/offer-service');
            const offerService = new OfferService(botInstance);
            
            // Crear datos de oferta
            const offerData = {
              asin: asin,
            ...
            // Publicar oferta automáticamente
            try {
              const result = await offerService.publishOffer(offerData);
              if (result.success) {
                console.log(`✅ Oferta automática publicada para ${asin}: ${price}€`);
              }
            } catch (offerError) {
              console.error('Error publicando oferta automática:', offerError);
            }
          }
        }
        
        // Enviar alerta si corresponde (código existente)
        if (oldPrice !== price && priceTracker.shouldSendAlert(product, price, oldPrice)) {
          
          // NUEVO: Verificar si pertenece a un grupo para alertas cruzadas
          const { ProductGroup } = require('../../models');
          
          // CORRECCIÓN: Usar el método estático findByASIN que sí existe
          const productGroup = await ProductGroup.findByASIN(asin);
          
          if (productGroup) {
            // Si pertenece a grupo, alertar a todos los usuarios de todas las variantes
            console.log(`📦 Producto pertenece al grupo: ${productGroup.productName}`);
            
            const allGroupProducts = await Product.find({ 
              asin: { $in: productGroup.variants } 
            });
            
            for (const groupProduct of allGroupProducts) {
              if (priceTracker.shouldSendAlert(groupProduct, price, oldPrice)) {
                // Determinar si es variante cruzada
                const isCrossVariant = groupProduct.asin !== asin;
                
                // Obtener colores usando el método del modelo
                const variantColor = productGroup.getVariantColor(asin);
                const userVariantColor = productGroup.getVariantColor(groupProduct.asin);
                
                priceTracker.emit('update', {
                  ...groupProduct.toObject(),
                  productId: groupProduct._id,
                  asin: groupProduct.asin,
                  oldPrice: oldPrice,
                  newPrice: price,
                  manualUpdate: true,
                  comment: comment,
                  ...
                  isGroupAlert: true,
                  groupName: productGroup.productName,
                  actualVariantASIN: asin,
                  actualVariantColor: variantColor,
                  userVariantColor: userVariantColor,
                  isCrossVariant: isCrossVariant
                });
    ...
    } // <- Llave de cierre del if (products.length > 0)
    
    // Respuesta de éxito (fuera del bucle)
...
const stats = async (ctx) => {
  ...
  try {
    ...
    const dashboard = `🔥 **Ofertrón Dashboard**
...
const marcarrobot = async (ctx) => {
  ...
  const args = ctx.message.text.split(' ');
  if (args.length < 2) {
    return ctx.reply('Uso: /marcarrobot ASIN [nombre_personalizado]\n\nEjemplos:\n• /marcarrobot B0DCVYS9FQ\n• /marcarrobot B0DCVYS9FQ "DREAME L40s Pro Ultra"');
  }
  
  const asin = args[1].toUpperCase();
  const customName = args.slice(2).join(' ').replace(/"/g, '');
  
  try {
    const { Product } = require('../../models');
    
    const updateData = { isRobotVacuum: true };
    ...
    let message = `✅ **Robot Aspirador Marcado**\n\n`;
    message += `📦 ASIN: \`${asin}\`\n`;
    message += `🤖 Productos marcados: **${result.modifiedCount}**\n`;
    message += `👥 Usuarios afectados: **${result.matchedCount}**\n`;
    
    if (customName) {
      message += `🏷️ Nombre personalizado: **${customName}**\n`;
    }
    
    message += `\n🔥 Ahora se publicarán ofertas automáticamente cuando baje el precio`;
...
const forzaroferta = async (ctx) => {
  ...
  try {
    ...
    console.log(`🔧 Iniciando forzado de oferta para ${asin}`);
    
    // CORRECCIÓN: Obtener la instancia del bot correctamente
    const Bot = require('../../bot');
    const botInstance = Bot.getBotInstance();
    
    if (!botInstance) {
      console.error('❌ No se puede obtener instancia del bot');
      return ctx.reply('❌ Error: No se puede obtener instancia del bot');
    }
    
    // CORRECCIÓN: Crear instancia del servicio con el bot completo
    const OfferService = require('../../services/offer-service');
    const offerService = new OfferService(botInstance);
    
    // Mostrar mensaje de procesamiento
    const processingMsg = await ctx.reply('🔄 Procesando oferta forzada...');
    
    // Ejecutar la publicación
    const result = await offerService.forcePublishOffer(asin);
    ...
    if (result.success) {
  let successMessage = `✅ **Oferta Publicada Forzadamente**\n\n`;
  successMessage += `📦 ${product.customName || product.name}\n`;
  successMessage += `💰 ${product.price}${product.currency}\n`;
  if (product.customName) {
    successMessage += `🏷️ Usando nombre personalizado\n`;
  }
  successMessage += `\n`;
      
      // Mostrar detalles de dónde se publicó
      if (result.results) {
        const publicationDetails = [];
        if (result.results.group && !result.results.group.error) {
          publicationDetails.push('✅ Grupo');
        } else if (result.results.group?.error) {
          publicationDetails.push(`❌ Grupo: ${result.results.group.error}`);
        }
        
        if (result.results.channel && !result.results.channel.error) {
          publicationDetails.push('✅ Canal');
        } else if (result.results.channel?.error) {
          publicationDetails.push(`❌ Canal: ${result.results.channel.error}`);
        }
        
        successMessage += `📡 **Publicación:**\n${publicationDetails.join('\n')}`;
      } else {
        successMessage += `🔥 Publicado en grupo y canal`;
      }
      
      await ctx.reply(successMessage, { parse_mode: 'Markdown' });
    } else {
      await ctx.reply(`❌ Error publicando oferta: ${result.error}`);
    }
    
  } catch (error) {
    console.error('Error en comando forzaroferta:', error);
    await ctx.reply(`❌ Error: ${error.message}`);
  }
};

const testconfiguracion = async (ctx) => {
  if (ctx.from.id !== ADMIN_USER_ID) {
    return ctx.reply('Comando no autorizado');
  }
  
  try {
    let diagnosticMessage = '🔧 DIAGNÓSTICO DE CONFIGURACIÓN\n\n';
    
    // Verificar variables de entorno
    diagnosticMessage += 'Variables de entorno:\n';
    diagnosticMessage += `✅ VACUUM_GROUP_ID: ${process.env.VACUUM_GROUP_ID || '❌ No configurado'}\n`;
    diagnosticMessage += `✅ VACUUM_CHANNEL_ID: ${process.env.VACUUM_CHANNEL_ID || '❌ No configurado'}\n`;
    diagnosticMessage += `✅ VACUUM_THREAD_ID: ${process.env.VACUUM_THREAD_ID || '⚠️ Opcional'}\n`;
    diagnosticMessage += `✅ AMAZON_TRACKING_TAG: ${process.env.AMAZON_TRACKING_TAG || '❌ No configurado'}\n`;
    
    // Verificar instancia del bot
    const Bot = require('../../bot');
    const botInstance = Bot.getBotInstance();
    diagnosticMessage += `\nBot Instance:\n`;
    diagnosticMessage += `✅ Bot disponible: ${botInstance ? 'Sí' : '❌ No'}\n`;
    diagnosticMessage += `✅ Telegram disponible: ${botInstance?.telegram ? 'Sí' : '❌ No'}\n`;
    
    // Probar crear OfferService
    try {
      const OfferService = require('../../services/offer-service');
      const offerService = new OfferService(botInstance);
      diagnosticMessage += `✅ OfferService: Creado correctamente\n`;
    } catch (error) {
      diagnosticMessage += `❌ OfferService: Error - ${error.message}\n`;
    }
    
    // ENVIAR SIN MARKDOWN PARA EVITAR ERRORES
    await ctx.reply(diagnosticMessage);
    
    // Intentar enviar mensaje de prueba al grupo
    if (process.env.VACUUM_GROUP_ID && botInstance) {
      try {
        const testOptions = {
          parse_mode: 'Markdown'
        };
        ...
        if (process.env.VACUUM_THREAD_ID) {
          testOptions.message_thread_id = parseInt(process.env.VACUUM_THREAD_ID);
        }
        
        const testMsg = await botInstance.telegram.sendMessage(
          process.env.VACUUM_GROUP_ID,
          '🧪 **Test de configuración del bot**\n\nSi ves este mensaje, la configuración básica funciona.',
          testOptions
        );
        
        await ctx.reply('✅ Mensaje de prueba enviado al grupo correctamente');
        
        // Eliminar mensaje de prueba después de 10 segundos
        setTimeout(async () => {
          try {
            await botInstance.telegram.deleteMessage(process.env.VACUUM_GROUP_ID, testMsg.message_id);
          } catch (e) {
            console.log('No se pudo eliminar mensaje de prueba');
          }
        }, 10000);
        
      } catch (error) {
        await ctx.reply(`❌ Error enviando mensaje al grupo: ${error.message}`);
      }
    }
    
  } catch (error) {
    console.error('Error en testconfiguracion:', error);
    await ctx.reply(`❌ Error en diagnóstico: ${error.message}`);
  }
};
