...
const importarTexto = async (ctx) => {
  ...
  const helpMessage = `📝 *IMPORTACIÓN POR TEXTO*

Envía precios en formato de texto, uno por línea:
\`\`\`
ASIN YYYY-MM-DD PRECIO
B0D9YHVZKS 2024-01-15 299.99
B0DBL9C6JY 2024-01-15 1199.00
B0DCVYS9FQ 2024-02-10 449.00
\`\`\`
...
Después envía el texto directamente y será procesado automáticamente.`;

  await ctx.replyWithMarkdown(helpMessage);
};

const procesarTextoMasivo = async (ctx) => {
  if (ctx.from.id !== ADMIN_USER_ID) {
    return;
  }
  ...
  // Detectar si es texto con formato de importación masiva
  // Debe tener al menos 2 líneas con formato ASIN FECHA PRECIO
  ...
  // Si al menos el 80% de las líneas tienen formato válido, procesar
  if (formatoValido < lines.length * 0.8) return;
  
  await ctx.reply('📥 Detectado formato de importación masiva. Procesando...');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (parts.length !== 3) {
      results.errors++;
      results.details.push(`Línea ${i + 1}: Formato incorrecto`);
      continue;
    }
    ...
    try {
      ...
      await PriceHistory.create({
        ...
        timestamp: timestamp,
        currency: '€',
        comment: 'Importado masivamente por texto'
      });
  ...
  let message = `📊 *IMPORTACIÓN DE TEXTO COMPLETADA*\n\n`;
...
const procesarArchivoCSV = async (ctx) => {
  if (ctx.from.id !== ADMIN_USER_ID) {
    return ctx.reply('Comando no autorizado');
  }
  
  if (!ctx.message.document) {
    return ctx.reply('Por favor, envía un archivo CSV');
  }
  
  const document = ctx.message.document;
  
  // Verificar que sea un archivo CSV
  if (!document.file_name?.endsWith('.csv') && document.mime_type !== 'text/csv') {
    return ctx.reply('El archivo debe ser un CSV (.csv)');
  }
  
  try {
    await ctx.reply('📥 Procesando archivo CSV...');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (parts.length !== 3) {
        results.errors++;
        results.details.push(`Línea ${i + 1}: Formato incorrecto (${line})`);
        continue;
      }
      
      const [asin, fecha, precioStr] = parts.map(p => p.trim());
      const precio = parseFloat(precioStr);
      
      // Validaciones
      if (!asin || asin.length !== 10) {
        results.errors++;
        results.details.push(`Línea ${i + 1}: ASIN inválido (${asin})`);
        continue;
      }
      
      if (isNaN(precio) || precio <= 0) {
        results.errors++;
        results.details.push(`Línea ${i + 1}: Precio inválido (${precioStr})`);
        continue;
      }
      ...
      if (!fechaRegex.test(fecha)) {
        results.errors++;
        results.details.push(`Línea ${i + 1}: Fecha inválida (${fecha}), usar YYYY-MM-DD`);
        continue;
      }
      ...
      try {
        ...
        await PriceHistory.create({
          ...
          timestamp: timestamp,
          currency: '€',
          comment: 'Importado masivamente desde CSV'
        });
        ...
        results.errors++;
        results.details.push(`Línea ${i + 1}: Error al guardar - ${error.message}`);
      }
    }
    
    // Respuesta con resultados
    let message = `📊 *IMPORTACIÓN COMPLETADA*\n\n`;
    message += `✅ Procesados correctamente: **${results.processed}**\n`;
    message += `❌ Errores: **${results.errors}**\n\n`;
    ...
    if (results.details.length > 0 && results.details.length <= 10) {
      ...
      message += `⚠️ Demasiados errores para mostrar (${results.details.length} total)\n`;
    }
    
    if (results.processed > 0) {
      message += `\n💡 *Recuerda:* Ejecuta /forzarrevision ASIN para cada producto actualizado`;
    }
...
module.exports = {
  ...
  ayudaanalytics,
  resumenbot,
  // NUEVAS FUNCIONES PARA IMPORTACIÓN MASIVA:
  procesarArchivoCSV,
  importarTexto,
  procesarTextoMasivo,
