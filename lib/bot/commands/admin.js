...
const limpiarstats = async (ctx) => {
  ...
  try {
    ...
    // Obtener todos los ASINs que existen en Product
    const validAsins = await Product.distinct('asin');
    
    // Buscar ProductStats que no tienen producto correspondiente
...
const broadcast = async (ctx) => {
  ...
  try {
    ...
    // Obtener todos los usuarios activos (que han usado el bot en últimos 90 días)
