...
module.exports = async ctx => {
  ...
  try {
    ...
    // Calcular precios mínimos para marcadores
    const periodMinPrice = Math.min(...history.map(h => h.price));
    const globalMinPrice = Math.min(...allHistory.map(h => h.price));
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      let marker = '';
      if (current.price === globalMinPrice) {
        marker = ' 🏆 PRECIO MÍNIMO HISTÓRICO';
      } else if (current.price === periodMinPrice) {
        marker = ' ✅';
      }
      
      historyText += `📅 ${date}: *${current.price}€*${duration}${marker}\n`;
    }
