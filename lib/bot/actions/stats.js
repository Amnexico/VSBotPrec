...
module.exports = async ctx => {
  ...
  try {
    ...
    const allHistory = await PriceHistory.find({ asin }).sort({ timestamp: -1 });
    const prices = allHistory.map(h => h.price).filter(p => p > 0);
    
    // FIX: El precio actual debe ser el MÁS RECIENTE del historial, NO el primero del array de precios
    const currentPrice = allHistory.length > 0 ? allHistory[0].price : product.price;
    ...
    // FIX: La fecha "desde" debe ser del registro MÁS ANTIGUO (último del array ordenado)
    const firstCheck = allHistory.length > 0 ? allHistory[allHistory.length - 1].timestamp : null;
    ...
    // FIX: Calcular precio anterior SOLO si es mayor que el actual (para mostrar tachado)
    let previousPrice = null;
    if (allHistory.length > 1) {
      // Buscar el precio anterior diferente al actual
      for (let i = 1; i < allHistory.length; i++) {
        if (allHistory[i].price !== currentPrice && allHistory[i].price > currentPrice) {
    ...
    if (previousPrice) {
      // FIX: Usar ~~ en lugar de ~~texto~~ para que funcione correctamente en Telegram
      currentPriceLine += ` (~${previousPrice.toFixed(2)}€~)`;
    }
    currentPriceLine += specialPriceText;
    
    // Generar historial SIN duración en paréntesis
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (current.price === minPrice) {
        ...
        // FIX: Solo marcar como "MÍNIMO ÚLTIMOS 30 DÍAS" si realmente está en los últimos 30 días
        if (current.timestamp >= last30Days) {
          marker = ' 🔥 MÍNIMO ÚLTIMOS 30 DÍAS';
        }
      }
      
      // FIX: Eliminar la duración en paréntesis
      historyText += `${date}: *${current.price.toFixed(2)}€*${marker}\n`;
    }
...
🛒 Ver en Amazon: https://amazon.es/dp/${asin}?tag=vsoatg-21
