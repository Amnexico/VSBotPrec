...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   // ACCIÓN PARA HISTORIAL INDIVIDUAL - VERSION DEBUG
this.action(/^history_(\w+)$/, async (ctx) => {
  console.log('=== DEBUG HISTORY ACTION ===');
  console.log('Match:', ctx.match);
  console.log('ProductId:', ctx.match[1]);
  ...
  try {
    ...
    console.log('Producto encontrado:', product ? 'SI' : 'NO');
    
    if (!product) {
      console.log('Producto no encontrado, retornando error');
    ...
    if (!asin) {
      console.log('ASIN no encontrado en producto');
      return ctx.editMessageText('Error: Producto sin ASIN', {
        ...Markup.inlineKeyboard([[Markup.button.callback('🔙 Volver al historial', 'menu_price_history')]])
      });
    }
    
    console.log('Buscando historial para ASIN:', asin);
    const history = await PriceHistory.find({ asin })
      .sort({ timestamp: -1 })
      .limit(10);
    
    console.log('Registros de historial encontrados:', history.length);
    
    if (history.length === 0) {
      console.log('Sin historial, mostrando mensaje');
      return ctx.editMessageText(`📊 ${product.name}\n\nSin historial disponible aún.`, {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([[Markup.button.callback('🔙 Volver al historial', 'menu_price_history')]])
      });
    }
    
    // Mensaje simple sin estadísticas complejas para evitar errores
    let message = `📊 ${product.name}\n\n`;
    message += `💰 Precio actual: ${product.price}€\n\n`;
    message += `📋 Últimos precios:\n`;
    
    history.slice(0, 3).forEach(record => {
      const date = record.timestamp.toLocaleDateString('es-ES');
      message += `${date}: ${record.price}€\n`;
    });
    
    console.log('Enviando mensaje:', message.substring(0, 100) + '...');
    ...
    console.log('=== ERROR EN HISTORY ACTION ===');
