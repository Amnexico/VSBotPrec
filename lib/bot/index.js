...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   // NUEVO: Menú automático para texto no-comando
   this.on('text', (ctx, next) => {
     ...
     // Mostrar menú para cualquier otro texto
   ...
   this.action('menu_my_products', async (ctx) => {
     ...
     if (products.length) {
       await ctx.editMessageText('Elige un producto de la lista:', 
 ...
 createProductListKeyboard(products) {
   const buttons = products.map(product => [
     Markup.button.callback(
       `${product.name.substring(0, 30)}... - ${product.price}€`,
       `!menu=${product.id}`
     )
   ]);
