 ...
 constructor(token, options) {
   ...
   this.action(/^history_(\w+)$/, async (ctx) => {
     try {
       ...
       // Crear un nuevo objeto match para la acción stats
       const originalMatch = ctx.match;
       const originalCallbackData = ctx.callbackQuery.data;
       
       // Simular que viene de stats con from_history=true
       ctx.match = [`!stats?id=${productId}&period=recent&from_history=true`, productId, '1', 'recent', 'true'];
       ctx.callbackQuery.data = `!stats?id=${productId}&period=recent&from_history=true`;
       
       // Llamar a la acción stats que ya funciona
       await actions.stats(ctx);
       
       // Restaurar valores originales por si acaso
       ctx.match = originalMatch;
       ctx.callbackQuery.data = originalCallbackData;
