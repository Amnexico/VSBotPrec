...
// Añadir estos handlers en lib/bot/index.js, en la sección de actions:
...
this.action(/^!list_page=(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // Contar total de productos del usuario
    const totalProducts = await Product.countDocuments({ user: user });
    const PRODUCTS_PER_PAGE = 8;
    const totalPages = Math.ceil(totalProducts / PRODUCTS_PER_PAGE);
    
    if (page < 1 || page > totalPages) {
      return ctx.answerCbQuery('Página no válida', true);
    }
    
    // Calcular paginación
    const skip = (page - 1) * PRODUCTS_PER_PAGE;
    
    // Obtener productos para la página
    const products = await Product.find({ user: user })
      .sort({ createdAt: -1 })
      .skip(skip)
    ...
    products.forEach((product, index) => {
      const productNumber = skip + index + 1;
      const displayName = product.name && product.name.length > 40 ? 
        escapeMarkdown(product.name.substring(0, 37)) + '...' : 
        escapeMarkdown(product.name || 'Producto sin nombre');
      
      message += `**${productNumber}.** ${displayName}\n`;
      message += `💰 ${product.price}${product.currency || '€'}`;
      
      if (product.preferences?.alertType) {
        if (product.preferences.alertType === 'percentage' && product.preferences.discountPercent) {
          message += ` | 🔔 ${product.preferences.discountPercent}% desc.`;
        } else if (product.preferences.alertType === 'custom' && product.preferences.targetPrice) {
          message += ` | 🎯 ${product.preferences.targetPrice}€`;
        } else if (product.preferences.alertType === 'any_drop') {
          message += ` | 📉 Cualquier bajada`;
        } else if (product.preferences.alertType === 'stock') {
    ...
    if (totalPages > 1) {
      message += `💡 Usa los botones para navegar entre páginas`;
    }
    
    // Actualizar mensaje
    await ctx.editMessageText(message, {
      parse_mode: 'Markdown',
      ...createProductListKeyboard(products, page, totalPages)
    });
...
// Handler para botón "noop" (no hace nada, solo muestra página actual)
