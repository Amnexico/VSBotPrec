...
this.use((ctx, next) => {
  ...
  // Si tiene parámetros, dejar que otros handlers lo procesen
   ...
   // Agrupación de ASIN
    this.command('agrupar', adminCommands.agrupar);
    this.command('desagrupar', adminCommands.desagrupar);
    this.command('vergrupos', adminCommands.vergrupos);
   ...
   this.command('resumenbot', adminCommands.resumenbot);
    ...
    // 🤖 Comandos de robots aspiradores - AÑADIR AQUÍ
    this.command('marcarrobot', adminCommands.marcarrobot);
    this.command('desmarcarrobot', adminCommands.desmarcarrobot);
    this.command('listarobots', adminCommands.listarobots);
    this.command('forzaroferta', adminCommands.forzaroferta);
    this.command('testconfiguracion', adminCommands.testconfiguracion);


    // 📢 Comandos de comunicación masiva - AÑADIR AQUÍ
    this.command('broadcast', adminCommands.broadcast);
    this.command('emailblast', adminCommands.emailblast);
...
console.log('DEBUG - startsWith check:', text.startsWith('/start verify_'));
     ...
     if (!hasProducts) {
       ...
       const welcomeText = '¡Hola! Soy OFERTRÓN by Vacuum Spain 👋\n\n' +
...
this.action(/^!list_page=(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // SOLO NOMBRES MÁS LARGOS - sin precios
    products.forEach(product => {
      const displayName = product.name ? 
...
function createProductListKeyboard(products, page, totalPages) {
  ...
  await ctx.editMessageText(message, {
   ...
   this.action('menu_help', (ctx) => {
     ctx.answerCbQuery();
     const helpText = '📋 **GUÍA COMPLETA - OFERTRÓN by @vacuumspain**\n\n' +
...
// Instancia global del bot para exportar
let botInstance = null;

// Método para obtener la instancia del bot
function getBotInstance() {
  return botInstance;
}

// Método para establecer la instancia del bot
function setBotInstance(instance) {
  botInstance = instance;
}

// Exportar con las nuevas funciones
module.exports = Bot;
