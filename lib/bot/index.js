...
this.use(async (ctx, next) => {
 ...
 // Handler para tracking de clicks de afiliado
this.action(/^amazon_click_(.+)$/, async (ctx) => {
  try {
    console.log('Amazon click handler ejecutado');
    const asin = ctx.match[1];
    console.log('ASIN:', asin);
    
    // Tracking del click con try/catch
    try {
      const AnalyticsService = require('../services/analytics-service');
      console.log('AnalyticsService importado');
      await AnalyticsService.trackAffiliateClick(ctx.from.id, asin);
      console.log('Tracking completado');
    } catch (analyticsError) {
      console.error('Error en analytics:', analyticsError.message);
    }
    
    // Redirigir a Amazon CON tu tag de afiliado
    const amazonUrl = `https://www.amazon.es/dp/${asin}?tag=vsoatg-21`;
    console.log('URL generada:', amazonUrl);
    
    await ctx.answerCbQuery('Redirigiendo a Amazon...', { 
      url: amazonUrl 
    });
    ...
    console.error('Error completo en amazon_click handler:', error);
    await ctx.answerCbQuery('Error al procesar click');
  }
});
