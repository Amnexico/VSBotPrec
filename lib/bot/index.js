...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   // ACCIÓN PARA HISTORIAL INDIVIDUAL - Implementación directa
   this.action(/^history_(\w+)$/, async (ctx) => {
     ...
     try {
       ...
       const daysDiff = firstCheck ? Math.ceil((Date.now() - firstCheck) / (1000 * 60 * 60 * 24)) : 0;
       ...
       let historyText = '📋 *Cambios recientes* (página 1):\n';
       ...
       recentHistory.forEach(record => {
         ...
         historyText += `${date}: *${record.price}€*${marker}\n`;
       });
...
${historyText}`;
       ...
       // Botones simplificados para historial desde menú principal
       const buttons = [
         [
           { text: '🔥 Recientes', callback_data: `!stats?id=${productId}&period=recent&from_history=true` },
           { text: '📅 Últimos 30 días', callback_data: `!stats?id=${productId}&period=month&from_history=true` },
           { text: '📚 Todos', callback_data: `!stats?id=${productId}&period=all&from_history=true` }
         ],
         [{ text: '🔙 Volver al historial', callback_data: 'menu_price_history' }]
       ];
       ...
       ctx.editMessageText(`Error al cargar historial: ${error.message}`, 
         Markup.inlineKeyboard([[Markup.button.callback('🔙 Volver al historial', 'menu_price_history')]]));
     }
   });
