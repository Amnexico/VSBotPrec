...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   this.action(/^history_(\w+)$/, async (ctx) => {
     ...
     // Simular el objeto ctx.callbackQuery para la función stats
     ctx.callbackQuery = {
       data: `!stats?id=${productId}&from_history=true`
     };
     
     // Usar la función de estadísticas existente
     return actions.stats(ctx);
   });
