...
// Añadir ANTES del handler de verificación (línea 46):
this.use((ctx, next) => {
  if (ctx.message?.text) {
...
// COMANDO PARA VERIFICACIÓN DE EMAIL
// Modificar el handler de verificación (línea 47):
this.command(/^\/start verify_(.+)$/, async (ctx) => {
  console.log('HANDLER DE VERIFICACIÓN EJECUTADO');
  console.log('Match completo:', ctx.match);
  console.log('ID extraído:', ctx.match[1]);
  
  const verifyUserId = parseInt(ctx.match[1]);
  const currentUserId = ctx.from.id;
  
  console.log('Verify ID:', verifyUserId, 'Current ID:', currentUserId);

  
  // Verificar que el usuario que hace clic es el mismo que se registró
  if (verifyUserId !== currentUserId) {
    return ctx.reply('❌ Este enlace de verificación no es válido para tu cuenta.');
  }
  ...
  try {
    ...
    if (userSettings.emailVerified) {
      return ctx.reply('✅ Tu email ya estaba verificado anteriormente.\n\n' +
                      `📧 ${userSettings.email} - Verificado correctamente`);
    }
    ...
    ctx.reply('🎉 ¡Email verificado correctamente!\n\n' +
             `✅ ${userSettings.email}\n` +
             `🔔 Ya puedes recibir alertas de precio por email\n\n` +
             `Usa /email para gestionar tu configuración.`,
      Markup.inlineKeyboard([
        [Markup.button.callback('📧 Ver configuración email', 'show_email_menu')],
        [Markup.button.callback('🔙 Menú principal', 'menu_main')]
      ])
    );
    
  } catch (error) {
    console.error('Error verificando email:', error);
    ctx.reply('❌ Error procesando la verificación. Inténtalo más tarde.');
  }
});
