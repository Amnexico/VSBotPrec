...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   this.action(/^history_(\w+)$/, async (ctx) => {
     ...
     // Crear el callbackQuery data correcto para la función stats
     const originalData = ctx.callbackQuery.data;
     ctx.callbackQuery.data = `!stats?id=${productId}&from_history=true`;
     
     try {
       return await actions.stats(ctx);
     } catch (error) {
       // Restaurar data original en caso de error
