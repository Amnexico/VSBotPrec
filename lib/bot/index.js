...
class Bot extends Telegraf {
 constructor(token, options) {
   ...
   // ACCIÓN PARA HISTORIAL INDIVIDUAL - Solución directa y funcional
   this.action(/^history_(\w+)$/, async (ctx) => {
     try {
       ...
       // Obtener historial
       const history = await PriceHistory.find({ asin: product.asin })
         .sort({ timestamp: -1 })
         .limit(10);
       ...
       // Crear mensaje
       let message = `📊 *Estadísticas de precio:* ${product.name}\n\n`;
       ...
       const buttons = [
         [
           Markup.button.callback('🔥 Recientes', `!stats?id=${productId}&period=recent&from_history=true`),
           Markup.button.callback('📅 30 días', `!stats?id=${productId}&period=month&from_history=true`),
           Markup.button.callback('📚 Todos', `!stats?id=${productId}&period=all&from_history=true`)
         ],
       ...
       return ctx.editMessageText('Error temporal. Inténtalo de nuevo.', {
         ...Markup.inlineKeyboard([[Markup.button.callback('🔙 Volver al historial', 'menu_price_history')]])
       });
     }
   });
