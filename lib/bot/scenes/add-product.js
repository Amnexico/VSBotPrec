...
const steps = [
  ...
  async ctx => {
    ...
    try {
  // Usar PA-API para obtener información del producto (maneja enlaces acortados)
  const productData = await paapiClient.getProductByUrl(url);
  
  // El ASIN viene en productData.asin
  const asin = productData.asin;
      
      const product = new Product({
        name: ctx.wizard.state.name,
        url: url,
        asin: productData.asin, // ASIN obtenido de PA-API
        user: user,
