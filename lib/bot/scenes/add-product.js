...
const { Product, UserSettings } = require('../../models');
...
const steps = [
  ...
  async ctx => {
    ...
    try {
      ...
      // VERIFICAR EMAIL INMEDIATAMENTE DESPUÉS DE AÑADIR PRODUCTO
      const userSettings = await UserSettings.findOne({ userId: user });
      const hasEmailConfigured = userSettings && userSettings.email;
      
      if (!hasEmailConfigured) {
        // Ofrecer configurar email si no está configurado (SIN importar número de productos)
        const emailOfferMsg = `✅ *Producto añadido correctamente*\n\n` +
          `📦 ${productData.name}\n` +
          ...
          `📧 *¿También quieres alertas por email?*\n` +
          `Es opcional pero te permitirá recibir alertas aunque no tengas Telegram abierto.`;
        
        // Keyboard combinado: alertas + email
        const combinedButtons = [];
        
        // Fila 1: 5% y 10%
        const price5 = productData.price ? (productData.price * 0.95).toFixed(2) : 'X';
        const price10 = productData.price ? (productData.price * 0.90).toFixed(2) : 'X';
        combinedButtons.push([
          Markup.button.callback(`🔵 5% (${price5}€)`, `setup_percent_5_${product.id}`),
          Markup.button.callback(`🟠 10% (${price10}€)`, `setup_percent_10_${product.id}`)
        ]);
        
        // Fila 2: 15% y 20%  
        const price15 = productData.price ? (productData.price * 0.85).toFixed(2) : 'X';
        const price20 = productData.price ? (productData.price * 0.80).toFixed(2) : 'X';
        combinedButtons.push([
          Markup.button.callback(`🔴 15% (${price15}€)`, `setup_percent_15_${product.id}`),
          Markup.button.callback(`🟣 20% (${price20}€)`, `setup_percent_20_${product.id}`)
        ]);
        
        // Fila 3: Opciones especiales
        combinedButtons.push([
          Markup.button.callback('⚡ Cualquier bajada', `setup_any_drop_${product.id}`),
          Markup.button.callback('🎯 Precio personalizado', `setup_custom_${product.id}`)
        ]);
        
        // Fila 4: Stock y configurar después
        combinedButtons.push([
          Markup.button.callback('📦 Solo cuando hay stock', `setup_stock_${product.id}`),
          Markup.button.callback('⏭️ Configurar después', `setup_later_${product.id}`)
        ]);
        
        // FILA 5: OPCIONES DE EMAIL (NUEVA)
        combinedButtons.push([
          Markup.button.callback('📧 Configurar email', `setup_email_from_product`),
          Markup.button.callback('📱 Solo Telegram', `skip_email_from_product`)
        ]);
        ...
        await ctx.replyWithMarkdown(emailOfferMsg, {
          ...Markup.inlineKeyboard(combinedButtons),
          disable_web_page_preview: true
        });
        
      } else {
        // Usuario YA tiene email - flujo normal sin ofrecer email
...
// ACCIONES PARA CONFIGURACIÓN DE EMAIL DESDE ADD-PRODUCT
scene.action('setup_email_from_product', async (ctx) => {
  await ctx.answerCbQuery();
  // Guardar contexto para volver después de configurar email
  ctx.wizard.state.returnTo = 'add_product';
  await ctx.scene.enter('setup-email');
});

scene.action('skip_email_from_product', async (ctx) => {
  await ctx.answerCbQuery();
  
  // Crear configuración sin email
