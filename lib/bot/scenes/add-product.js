...
// Crear el producto en la base de datos
      ...
      // NUEVO: Guardar precio inicial en historial SOLO si es el primer usuario que añade este ASIN
      if (productData.price && productData.price > 0) {
        const { PriceHistory } = require('../../models');
        
        // Verificar si ya existe historial para este ASIN
        const existingHistory = await PriceHistory.findOne({ asin: productData.asin });
        
        if (!existingHistory) {
          // Solo guardar si es la primera vez que se añade este ASIN
  ...
  console.log(`Precio inicial guardado en historial: ${productData.asin} - ${productData.price}€`);
}
      await product.save();
