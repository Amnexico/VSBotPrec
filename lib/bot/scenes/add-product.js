...
scene.action(/^setup_later_(\w+)$/, async (ctx) => {
  ...
  // Verificar si el usuario ya tiene configurado email
  ...
  if (!hasEmailConfigured) {
    // Ofrecer configurar email si es su primer producto (INCLUYE ADMIN para testing)
    const userProducts = await Product.countDocuments({ user: ctx.from.id });
    
    if (userProducts <= 1) { // Primer o segundo producto
      const offerEmailMsg = `✅ *Producto añadido correctamente*\n\n` +
        `¿Te gustaría recibir alertas también por email?\n\n` +
        `📧 *Beneficios del email:*\n` +
        `• No necesitas tener Telegram siempre abierto\n` +
        `• Enlaces directos a Amazon\n` +
        `• Historial de alertas en tu correo\n\n` +
        `Es completamente opcional 😊`;
      
      await ctx.editMessageText(offerEmailMsg, {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
          [Markup.button.callback('📧 Configurar email', 'setup_email_from_product')],
          [Markup.button.callback('⏭️ Solo Telegram por ahora', 'skip_email_from_product')],
          [Markup.button.callback('📋 Ver mis productos', 'menu_my_products')]
        ])
      });
  ...
  // Usuario ya tiene email o no es su primer producto - continuar normal
