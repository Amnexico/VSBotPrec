'use strict';
const { Product, PriceHistory, ProductGroup } = require('../models');
      ...
      if (oldPrice !== newPrice) {
        ...
        if (isPriceDrop && product.isRobotVacuum && this.offerService) {
          try {
            ...
            const offerData = {
              ...
              name: product.customName || product.name, // Usar nombre personalizado si existe
              oldPrice: oldPrice,
              newPrice: newPrice,
            ...
            const offerResult = await this.offerService.publishOffer(offerData);
            
            if (offerResult.success) {
              logger.info(`✅ Oferta publicada automáticamente: ${product.name}`);
            } else {
              logger.info(`ℹ️ Oferta no publicada: ${offerResult.error || 'No cumple criterios'}`);
            }
        ...
        if (isPriceDrop || userWantsAllAlerts) {
          if (this.shouldSendAlert(product, newPrice)) {
            ...
            // NUEVO: Verificar si pertenece a un grupo
            const productGroup = await ProductGroup.findByASIN(product.asin);
            ...
            if (productGroup) {
              ...
              for (const groupProduct of allGroupProducts) {
                if (this.shouldSendAlert(groupProduct, newPrice)) {
                  // Tracking de alerta enviada
                  const AnalyticsService = require('../services/analytics-service');
                  await AnalyticsService.trackAlertSent(
                    groupProduct.user, 
                    groupProduct.asin,
                    groupProduct.preferences?.alertType || 'percentage'
                  );
                  
                  // Determinar si es la misma variante o variante cruzada
                  const isCrossVariant = groupProduct.asin !== product.asin;
                  const variantColor = productGroup.getVariantColor(product.asin);
                  ...
                  this.emit('update', {
                    ...
                    actualVariantASIN: product.asin,
              ...
              // Alerta individual normal (código existente)
              const AnalyticsService = require('../services/analytics-service');
              await AnalyticsService.trackAlertSent(
                product.user, 
